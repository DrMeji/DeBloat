...
const ultimateExtra: Tweak[] = [
  ...
  // ============================================================
  {
    ...
    target: [
      ...
      "Say 'Applying Defender policies (registry)...'",
      ...
      "New-Item 'HKLM:\\SOFTWARE\\Microsoft\\Windows Defender\\Features' -Force | Out-Null",
      "New-Item 'HKLM:\\SOFTWARE\\Policies\\Microsoft\\Windows Defender Security Center\\Systray' -Force | Out-Null",
      ...
      "Say 'Disabling Defender services (sc.exe, non-blocking)...'",
      "foreach ($n in @('WinDefend','WdNisSvc','Sense','MDCoreSvc')) { sc.exe config $n start= disabled 2>$null | Out-Null; sc.exe stop $n 2>$null | Out-Null; if (Test-Path \"HKLM:\\SYSTEM\\CurrentControlSet\\Services\\$n\") { Set-ItemProperty \"HKLM:\\SYSTEM\\CurrentControlSet\\Services\\$n\" Start 4 -Type DWord -Force }; Say \"  $n set disabled\" }",
      "Say 'Keeping SecurityHealthService / wscsvc (Task Manager)...'",
      "foreach ($n in @('SecurityHealthService','wscsvc')) { sc.exe config $n start= demand 2>$null | Out-Null; sc.exe start $n 2>$null | Out-Null; Say \"  $n manual\" }",
      "Say 'Stopping Defender processes...'",
      "Get-Process -Name MsMpEng,NisSrv,smartscreen -ErrorAction SilentlyContinue | Stop-Process -Force -ErrorAction SilentlyContinue",
      ...
      "Say 'Defender engine disabled (reboot recommended)'; exit 0",
    ].join('; '),
    undo: `$ErrorActionPreference='SilentlyContinue'; Remove-ItemProperty 'HKLM:\\SOFTWARE\\Policies\\Microsoft\\Windows Defender' -Name DisableAntiSpyware -ErrorAction SilentlyContinue; Remove-ItemProperty 'HKLM:\\SOFTWARE\\Policies\\Microsoft\\Windows Defender' -Name DisableAntiVirus -ErrorAction SilentlyContinue; foreach ($exe in @('MsMpEng.exe','NisSrv.exe','SecurityHealthService.exe','SecurityHealthSystray.exe','smartscreen.exe','SecurityHealthHost.exe','Taskmgr.exe')) { Remove-Item \"HKLM:\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Image File Execution Options\\$exe\" -Recurse -Force -ErrorAction SilentlyContinue }; foreach ($n in @('WinDefend','WdNisSvc','Sense','MDCoreSvc','SecurityHealthService','wscsvc')) { sc.exe config $n start= demand 2>$null | Out-Null }; exit 0`,
  },
  {
