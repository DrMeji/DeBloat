...
/** Default landscape size — clamped down on portrait / small displays. */
const PREFERRED_WIDTH = 1500;
...
/**
 * Fit the window inside a display's work area (taskbar excluded).
 * Handles portrait secondaries and bad/zero metrics without clipping UI.
 */
function fitWindowToDisplay(display = getTargetDisplay()) {
  const work = display.workArea;
  const bounds = display.bounds;
  const areaW = Math.max(work?.width || 0, bounds?.width || 0, MIN_WIDTH);
  const areaH = Math.max(work?.height || 0, bounds?.height || 0, MIN_HEIGHT);
  const originX = Number.isFinite(work?.x) ? work.x : (bounds?.x ?? 0);
  const originY = Number.isFinite(work?.y) ? work.y : (bounds?.y ?? 0);

  const width = Math.min(
    PREFERRED_WIDTH,
    Math.max(MIN_WIDTH, areaW - DISPLAY_MARGIN * 2)
  );
  const height = Math.min(
    PREFERRED_HEIGHT,
    Math.max(MIN_HEIGHT, areaH - DISPLAY_MARGIN * 2)
  );

  return {
    width,
    height,
    x: Math.round(originX + (areaW - width) / 2),
    y: Math.round(originY + (areaH - height) / 2),
    maxWidth: areaW,
    maxHeight: areaH,
  };
}

function clampWindowToCurrentDisplay(win: BrowserWindow) {
  const display = screen.getDisplayMatching(win.getBounds());
  const fit = fitWindowToDisplay(display);
  const current = win.getBounds();
  const width = Math.min(current.width, fit.width);
  const height = Math.min(current.height, fit.height);
  // Re-center if we had to shrink (e.g. dragged onto a vertical monitor)
  const needsShrink = width < current.width || height < current.height;
  win.setBounds({
    width,
    height,
    x: needsShrink ? fit.x : current.x,
    y: needsShrink ? fit.y : current.y,
  });
...
function showWindow() {
  ...
  clampWindowToCurrentDisplay(mainWindow);
  mainWindow.show();
  mainWindow.focus();
}

function createWindow() {
  const fit = fitWindowToDisplay();
  ...
  // If the user drags onto a portrait / smaller screen, shrink to fit.
...
function buildAppxRemoveCommand(target: string, permanent?: boolean): string {
  ...
  for (const name of names) {
    ...
    parts.push(`Write-Output "Looking for app: ${safe}"`);
    // Current-user packages first (most reliable Remove-AppxPackage path)
    parts.push(
      `$pkgs = @(Get-AppxPackage '*${safe}*' -ErrorAction SilentlyContinue); ` +
      `if ($pkgs.Count -eq 0) { Write-Output '  (not installed for current user)' } ` +
      `else { foreach ($p in $pkgs) { Write-Output ("  Removing " + $p.PackageFullName); ` +
      `Remove-AppxPackage -Package $p.PackageFullName -ErrorAction SilentlyContinue } }`
    );
    // All-users packages (Win11 often keeps stubs for other users / provisioned copies)
    parts.push(
      `$all = @(Get-AppxPackage -AllUsers '*${safe}*' -ErrorAction SilentlyContinue); ` +
      `foreach ($p in $all) { Write-Output ("  Removing all-users " + $p.PackageFullName); ` +
      `try { Remove-AppxPackage -Package $p.PackageFullName -AllUsers -ErrorAction Stop } ` +
      `catch { Remove-AppxPackage -Package $p.PackageFullName -ErrorAction SilentlyContinue } }`
    );
    if (permanent) {
      parts.push(
        `Get-AppxProvisionedPackage -Online -ErrorAction SilentlyContinue | ` +
        `Where-Object { $_.DisplayName -like '*${safe}*' -or $_.PackageName -like '*${safe}*' } | ` +
        `ForEach-Object { Write-Output ("  Deprovisioning " + $_.PackageName); ` +
        `Remove-AppxProvisionedPackage -Online -PackageName $_.PackageName -ErrorAction SilentlyContinue | Out-Null }`
      );
      parts.push(
        `winget uninstall --name '${safe}' --silent --accept-source-agreements --disable-interactivity 2>$null | Out-Null`
      );
    }
  }
  parts.push('exit 0');
...
function createQuietOutputPump(
  ...
  const isProgressBar = (line: string) =>
    /[█▒░#]/.test(line) ||
    /\d+(\.\d+)?\s*MB\s*\/\s*\d+(\.\d+)?\s*MB/i.test(line) ||
    /^\s*\d+%\s*$/.test(line) ||
    /%\s*(Total|Received|Xferd)/i.test(line);
  ...
  const emitProgress = (line: string) => {
    const mbMatch = line.match(/([\d.]+)\s*MB\s*\/\s*([\d.]+)\s*MB/i);
    if (mbMatch) {
      const cur = parseFloat(mbMatch[1]);
...
function buildDirectInstallCommand(app: {
  ...
  return (
    ...
    `Write-Output '${url}'; ` +
    `& curl.exe -L --fail --progress-bar --retry 3 --connect-timeout 30 -o $out '${url}'; ` +
